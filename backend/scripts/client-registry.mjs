...
export function validateClient(manifest) {
  ...
  if (!["onboarding", "active", "suspended", "archived"].includes(manifest.status)) {
    errors.push("status is invalid");
  }
  if (!httpsUrl(manifest.domains?.production)) errors.push("production domain must be HTTPS");
  ...
  if (!httpsUrl(manifest.supabase?.url)) errors.push("Supabase URL must be HTTPS");
...
export function runVercel(args, options = {}) {
  ...
  const commandArgs = npxCli && existsSync(npxCli)
